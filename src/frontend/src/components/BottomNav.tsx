...
function ChatTabIcon({ isActive }: { isActive: boolean }) {
  ...
  return (
    <div className="relative">
      ...
      {count > 0 && (
        <span className="absolute -top-1.5 -right-1.5 min-w-[16px] h-4 bg-destructive text-white text-[9px] font-bold rounded-full flex items-center justify-center px-0.5 leading-none">
          {count > 99 ? "99+" : count}
        </span>
      )}
    </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              data-ocid={item.ocid}
              onClick={() => onNavigate(item.id)}
              className={`flex flex-col items-center gap-0.5 px-2 py-2 min-w-[44px] transition-all ${
